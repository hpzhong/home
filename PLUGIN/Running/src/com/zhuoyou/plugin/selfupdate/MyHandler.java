...
public class MyHandler extends Handler {
				...
				if (f.exists()) {
					...
					try {
						...
						if (v < Integer.parseInt(version)) {
							showAlertDialog(policy , path, data, true);
						}
					} catch (NameNotFoundException e) {
						e.printStackTrace();
					}
				} else {
					showAlertDialog(policy,sdcardPath, data, false);
				}
			}
			break;
		default:
	...
	private void showAlertDialog(int policy , final String sdcardPath, final String data, final boolean exsit) {
		/**
		 * show alert dialog
		 */
		int yes_id = 0;
