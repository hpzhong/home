...
public class MotionDataActivity extends Activity implements View.OnClickListener {

	private Context mCtx = RunningApp.getInstance();
	public static MotionDataActivity mInstance = null;
    ...
    private NewTextView goalCalorieText;
	...
	public void initView() {
        ...
        goalCalorieText = (NewTextView) findViewById(R.id.text_goal_calorie);
	...
	public void onResume() {
		...
		initData();
		setView();
		RunningTitleBar.getTitleBar(this, getResources().getString(R.string.data_center));
	}
	...
	public void onClick(View v) {
		...
		case R.id.dc_linearlayout_user_info:
			startActivity(new Intent(mCtx, PersonalConfigActivity.class));
			break;
		...
		case R.id.dc_relativelayout_goal:
			startActivity(new Intent(mCtx, PersonalGoalActivity.class));
			break;
	...
	private void setViewGoalSetting() {
		int goalCal = mPersonalGoal.mGoalCalories;
		goalCalorieText.setTypeface(mNewtype);
		if(goalCalorieText.refreshDisabled) {
			goalCalorieText.setValue(goalCal);
			goalCalorieText.setText((new StringBuilder()).append(goalCal).toString());
		} else {
			goalCalorieText.setValue(goalCal);
		}
	...
	private void setViewBestStep() {
        ...
        bestStepText.setTypeface(mNewtype);
	...
	private void setViewAvgStep() {
        ...
        avgStepText.setTypeface(mNewtype);
        bestStepLayout.setVisibility(View.GONE);
        Animation animation = AnimationUtils.loadAnimation(mCtx, R.anim.fade_in);
        animation.setAnimationListener(new Animation.AnimationListener() {
	...
	private void setViewBestCalorie() {
        ...
        bestCalText.setTypeface(mNewtype);
	...
	private void setViewAvgCal() {
        ...
        avgCalText.setTypeface(mNewtype);
        bestCalorieLayout.setVisibility(View.GONE);
        Animation animation = AnimationUtils.loadAnimation(mCtx, R.anim.fade_in);
        animation.setAnimationListener(new Animation.AnimationListener() {
	...
	private void setViewTotalKM() {
		...
		totalStepText.setTypeface(mNewtype); 
		if (d < 0.0001D) {
	...
	private void setViewTotalCal() {
		...
		totalCalText.setTypeface(mNewtype);
	...
	private void initListener() {
		mNewScrollView.addListener(goalCalorieText);
	...
	private final Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
            if(!goalCalorieText.refreshDisabled) {
                onMeasureTxt(goalCalorieText);
                canSendScroll = true;
            }
            if(!totalCalText.refreshDisabled) {
                onMeasureTxt(totalCalText);
                canSendScroll = true;
            }
            if(!BMIText.refreshDisabled) {
                onMeasureTxt(BMIText);
                canSendScroll = true;
            }
            if(!BMRText.refreshDisabled) {
                onMeasureTxt(BMRText);
                canSendScroll = true;
            }
            if(!weightText.refreshDisabled) {
                onMeasureTxt(weightText);
                canSendScroll = true;
            }
            if(isFirstProgressCal) {
                onMeasureLayout(progressCal);
                canSendScroll = true;
            }
            if(canSendScroll)
                mNewScrollView.sendScroll(1, 0);
		}
	};
