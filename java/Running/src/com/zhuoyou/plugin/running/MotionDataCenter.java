...
public class MotionDataCenter {
	...
	public MotionDataCenter(Context context) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (date_list != null && date_list.size() > 0 && date_list.indexOf(day) != -1) {
				initAddSport(day);
				Cursor c = cr.query(DataBaseContants.CONTENT_URI, new String[] { "_id", "steps", "calories", "kilometer", "complete" }, DataBaseContants.DATE + "  = ? AND " + DataBaseContants.STATISTICS + " = ? ", 
				...
				if (c.getCount() > 0 && c.moveToFirst() && c.getLong(c.getColumnIndex(DataBaseContants.ID)) > 0) {
					...
					complete.add(String.valueOf(0));
				}
				c.close();
				c = null;
			} else {
				steps.add(String.valueOf(0));
				calories.add(String.valueOf(0));
				kilometers.add(String.valueOf(0));
				complete.add(String.valueOf(0));
			}
		}
	...
	public String getGoal() {
		...
		if (complete != null && complete.size() > 0) {
			...
			double d = (double)day / (double)complete.size();
