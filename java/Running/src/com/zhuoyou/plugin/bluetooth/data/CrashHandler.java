...
public class CrashHandler implements UncaughtExceptionHandler {
	...
	private String saveCrashInfo2File(Throwable ex, String logPath) {
		...
		try {
			...
			String fileName = mContext.getPackageName() + "_crash-" + time
					+ "-" + timestamp +"-" + infos.get("versionCode") + ".log";
...
//		UploadInfo uploadInfo = getUploadInfo(ctx);
		if (reportFilesNames != null && reportFilesNames.length > 0) {
			...
			for (int i = 0; i < reportFilesNames.length; i++) {
				File file = new File(SD_CARD_PATH + LOG_DIR,
						reportFilesNames[i]);
...
//			String fileIndexs = UpLoadFilesUtil.postReport(ctx,
//					UpLoadFilesUtil.getUploadparams(ctx, uploadInfo), files);
