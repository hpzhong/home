...
public class VCardComposer {
    ...
    private static final HashMap<Integer, String> orgTypeMap = new HashMap<Integer, String>();
    ...
    static {
        ...
        orgTypeMap.put(Contacts.Phones.TYPE_WORK, "WORK");
        orgTypeMap.put(Contacts.Phones.TYPE_OTHER, "X-OTHER");
    }
    ...
    public String createVCard(ContactStruct struct, int vcardversion)
        ...
        if(struct.organizationList!=null){
        	appendContactOrganizationStr(struct.organizationList, vcardversion);
        }
    ...
    private void appendContactMethodStr(
        ...
        for (ContactStruct.ContactMethod contactMethod : contactMList) {
            ...
            case Contacts.KIND_POSTAL:
                ...
                break;
            case Contacts.KIND_NICKNAME:
                if (!isNull(contactMethod.data)) {
                    mResult.append("NICKNAME:").append(
                            foldingString(contactMethod.data, version)).append(
                            mNewline);
                }
                break;
            case Contacts.KIND_WEBSITE:
                if (!isNull(contactMethod.data)) {
    ...
    private void appendContactOrganizationStr(List<OrganizationData> contactOList, int version){
    	HashMap<String, String> numMap = new HashMap<String, String>();
        String joinMark = version == VERSION_VCARD21_INT ? ";" : ",";

        for (ContactStruct.OrganizationData org : contactOList) {
            String type;
            if (!isNull(org.companyName)) {
                type = getOrgTypeStr(org);
                if (version == VERSION_VCARD30_INT && type.indexOf(";") != -1) {
                    type = type.replace(";", ",");
                }
                if (numMap.containsKey(org.companyName)) {
                    type = numMap.get(org.companyName) + joinMark + type;
                }
                numMap.put(org.companyName, type);
            }
        }
    ...
    private String getOrgTypeStr(ContactStruct.OrganizationData org) {

//        int orgType = org.type;
//        String typeStr, label;
//
//        if (orgTypeMap.containsKey(orgType)) {
//            typeStr = orgTypeMap.get(orgType);
//        } else if (orgType == Contacts.Phones.TYPE_CUSTOM) {
//            label = org.positionName.toUpperCase();
//                typeStr = "X-CUSTOM-" + label;
//        } else {
//            typeStr = "WORK"; // the default type is VOICE in spec.
//        }
        return "WORK";
    }
}
