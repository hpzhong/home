...
public class PolylineChart extends View {
	...
	public PolylineChart(Context paramContext, ArrayList<Double> paramArrayList, int paramInt1, int paramInt2, int paramInt3) {
		...
		mPaint = new Paint(1);
		mCornerPathEffect = new CornerPathEffect(40.0F);
	...
	protected void onDraw(Canvas paramCanvas) {
		...
		for (int i = 1; i < dataList.size(); i++) {
			float f4 = f2 + Tools.dip2px(context, 45.0F);
		...
		for (int k = 0; k < XList.size(); k++) {
			paramCanvas.drawCircle(((Float)XList.get(k)).floatValue(), ((Float)YList.get(k)).floatValue(), Tools.dip2px(context, 7.0F), localPaint);
			paramCanvas.drawCircle(((Float)XList.get(k)).floatValue(), ((Float)YList.get(k)).floatValue(), Tools.dip2px(context, 8.0F), mPaint);
		}
	}
}
